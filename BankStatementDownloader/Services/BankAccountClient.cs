...
namespace BankStatementDownloader.Services
{
    ...
    {
        private IBankApiClient _bankApiClient;
        private IConfiguration _configuration;
        private ILogger<BankAccountClient> _logger;
        private string _xsrfToken;

        public BankAccountClient(IBankApiClient bankApiClient,  IConfiguration configuration, ILogger<BankAccountClient> logger)
        {
        ...
        public async Task<LoginStep2Response> Login()
        {
            ...
            {
                // Login step: 1to2
                var loginStep1Response = await _bankApiClient.PostAsync($"/frontend-web/app/j_spring_security_check?step=1to2&j_username={_configuration["BankApi:Username"]}", null);
                ...
                var loginStep2Response = await _bankApiClient.PostAsync($"frontend-web/app/j_spring_security_check?step=2&j_username={_configuration["BankApi:Username"]}&j_password={maskedPassword}", null);
                ...
                await _bankApiClient.SetDefaultRequestHeadersAsync(new Dictionary<string, string>() { { "X-XSRF-TOKEN", _xsrfToken } });
        ...
        public async Task<BankStatementsList> GetBankStatementsList(string accountId, DateTime dateFrom, DateTime dateTo, bool fetchAll)
        {
            try
            {
                var bankStatementsListResponse = await _bankApiClient.GetAsync($"/frontend-web/api/account_statement?accountId={accountId}&customerId={_configuration["BankApi:Username"]}&dateFrom={dateFrom.ToString("yyyy-MM-dd")}&dateTo={dateTo.ToString("yyyy-MM-dd")}&fetchAll={fetchAll}");
        ...
        public async Task<FileContentResult> DownloadBankStatement(string formatType, string contentDisposition, string language, int statementId, bool defaultFlag, string extendedFormatType)
        {
            try
            {
                var bankStatementResponse = await _bankApiClient.GetAsync($"/frontend-web/api/account_statement/downloads/download_statement_list_report.json?formatType={formatType}&contentDisposition={contentDisposition}&language={language}&statementId={statementId}&default={defaultFlag}&extendedFormatType={extendedFormatType}");
        ...
        public async Task Logout()
        {
            try
            {
                var bankStatementsListResponse = await _bankApiClient.GetAsync($"/frontend-web/app/logout.html?X-XSRF-TOKEN={_xsrfToken}");
                bankStatementsListResponse.EnsureSuccessStatusCode();
            }
            catch (Exception ex)
            {
